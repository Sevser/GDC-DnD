...
export default defineComponent({
  ...
  computed: {
    sortFields() {
      return this.$store.state.spells.sort.available.map((sort: Sorting) => sort.forParams.sort);
    },
  },
  mounted() {
    if (this.$store.state.spells.sort.current) {
      this.current = this.$store.state.spells.sort.current.forParams.sort;
    }
  },
  methods: {
    ...
    updateCurrent(newValue: string) {
      ...
      this.$store.commit('spells/setCurrentSorting', newValue);
    },
  },
});
</script>
